...
// const VideoPlayer: React.FC<{ stream: MediaStream; username: string; isMuted?: boolean }> = ({ stream, username, isMuted }) => {
...
//         newSocket.on('session_status_updated', ({ type, status }) => {
//             if (type === 'live') setIsLive(status);
//             if (type === 'chat') setIsChatEnabled(status);
//         });
...
//     // --- EFFET 2 : Démarrage Média et PeerJS (LORSQUE currentUser EST PRÊT) ---
//     useEffect(() => {
//         // CORRECTION CLÉ : Ce bloc ne s'exécute que lorsque socket ET currentUser sont disponibles.
...
//                     console.log('PeerJS ouvert. Envoi de join_class_video avec:', currentUser);
...
//     }, [socket, currentUser, sessionId]); // Dépendance explicite à currentUser et sessionId
...
//                 console.log(`Tentative d'appel vers ${peerId}`);
...
//             if (!roomState.participants[callId]) {
//                 callsRef.current[callId].close();
//                 delete callsRef.current[callId];
//             }
//         });
...
//                 <main className="flex-grow p-2 md:p-4 bg-gray-900">
//                     <div className="h-full w-full grid grid-cols-4 grid-rows-4 gap-2">
//                         <div className="relative col-span-4 row-span-4 md:col-span-3 rounded-lg bg-black">
...
//                             ) : (
//                                 <div className="w-full h-full flex items-center justify-center"><p>En attente du professeur...</p></div>
//                             )}
//                         </div>
//                         <div className="hidden md:flex col-span-1 row-span-4 flex-col gap-2 overflow-y-auto">
//                             {currentUser?.role === 'STUDENT' && myStream && (
//                                 <div className="relative aspect-video rounded-lg bg-black">
//                                     <VideoPlayer stream={myStream} username={userFullName} isMuted />
//                                 </div>
//                             )}
...
//                                 return (
//                                     <div key={id} className="relative aspect-video rounded-lg bg-black">
//                                         <VideoPlayer stream={stream} username={`${user.firstName} ${user.lastName}`} />
//                                     </div>
//                                 );
//                             })}
//                         </div>
//                     </div>
//                 </main>
//                 <footer className="flex-shrink-0 bg-gray-950/80 p-3 flex items-center justify-between h-28 border-t border-gray-700">
//                     <div className="flex gap-4 overflow-x-auto">
//                         {nonSpeakingParticipants.map(user => (
//                             <ParticipantAvatar key={user.id} user={user} isHandRaised={roomState?.handRaises.includes(user.id) || false} onInterrogate={() => handleInterrogate(user.id)} isTeacherView={currentUser?.role === 'TEACHER'} />
//                         ))}
//                     </div>
//                 </footer>
