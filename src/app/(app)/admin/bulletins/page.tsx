...
import { 
    ...
    Phone, FileBadge, Stamp, Loader2,AlertTriangle, XCircle
} from 'lucide-react';
import { listClasses, generateClassBulletins, saveBulletinConfig } from '@/services/api';
...
export default function BulletinBuilderPage() {
    ...
    // --- NOUVEAUX ÉTATS POUR GÉRER L'ERREUR ---
    ...
    const handlePrint = useReactToPrint({
        ...
        documentTitle: `Bulletins_${selectedClass || "Demo"}`,
    });
    ...
    //     const token = Cookies.get('token');
    //     try {
    //         const res = await generateClassBulletins(selectedClass, token!);
    //         // @ts-ignore
    //         setBulletins(res.data.bulletins.map(b => ({...b, decision: ""})));
    //         // @ts-ignore
    ...
    //         setConfig(prev => ({ 
    //             ...prev, 
    //             authNumber: res.data.establishment.authNumber || prev.authNumber,
    //             phoneNumber: res.data.establishment.phone || prev.phoneNumber
    //         }));
    //     } catch (e) {
    //         console.error(e);
    //     } finally {
    ...
    const handleGenerate = async () => {
        if(!selectedClass) return;
        setLoading(true);
        setMissingData([]); // Reset des erreurs
        ...
        try {
            ...
            console.error(error);
            // VÉRIFICATION DE L'ERREUR NOTES MANQUANTES
            if (error.response && error.response.status === 400 && error.response.data.code === 'MISSING_GRADES') {
                ...
                setShowMissingModal(true); // On ouvre le popup
            } else {
    ...
    return (
        <div className="flex h-screen bg-gray-900 text-white overflow-hidden font-sans">
            ...
            {/* SIDEBAR */}
            <div className="w-80 bg-gray-800 border-r border-gray-700 flex flex-col h-full shadow-2xl z-20">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            {config.bgType === 'GRADIENT' && (
                                <div>
                                    <label className="text-[10px] text-gray-400 block mb-1">Couleur 2</label>
                                    <input type="color" className="w-full h-6 cursor-pointer rounded bg-transparent" value={config.bgColor2} onChange={e => setConfig({...config, bgColor2: e.target.value})} />
                                </div>
                            )}
                            <div>
                        ...
                        <div>
                            <label className="text-[10px] text-gray-400 block mb-1">Police</label>
                            <select 
                                className="w-full p-1 bg-gray-700 border border-gray-600 rounded text-xs"
                            ...
                            >
                                ...
                                <option value="ui-serif">Serif (Officiel)</option>
                ...
                <div className="p-4 border-t border-gray-700 bg-gray-800">
                    <button 
                        ...
                        className="w-full py-3 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-lg font-bold shadow-lg hover:from-green-600 hover:to-emerald-700 flex items-center justify-center gap-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                    >
